...
class CQLConstants(unittest.TestCase):
    ...
    def test____assumptions(self):
        ...
        ae(constants.SIMPLE_SQUARE_DESIGNATOR,
           r'[a-h](?:-[a-h])?[1-8](?:-[1-8])?')
        ae(constants.COMPOUND_SQUARE_DESIGNATOR,
           r''.join(
        ...
        ae(constants.PIECE_DESIGNATOR,
           r''.join((r'(?:(?:[a-h](?:-[a-h])?[1-8](?:-[1-8])?|\[[a-h]',
                     ...
                     r'[KQRBNPkqrbnpAa_]\b')))
        if pgn_read_constants is None:
            ae(len([c for c in dir(constants)
                                  if not c.startswith('__')]), 39)
        else:
            ae(len([c for c in dir(constants)
                                  if not c.startswith('__')]), 35)
